...
def title(msg):
    ...
    chromedriver_dir = r"C:\Users\sohan\Desktop\chromedriver.exe"
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity = discord.Game("이예와 노래 듣기"))
...
async def 나가(ctx):
    try:
        ...
        await ctx.send('이미 그 채널에 들어가 있지 않아요..')
...
async def 일시정지(ctx):
    if vc.is_playing():
        vc.pause()
        await ctx.send(embed = discord.Embed(title= "일시정지", description = musicnow[0] + "을(를) 일시정지 했습니다.", color = 0xe9c949))
    else:
...
async def 다시재생(ctx):
    try:
        vc.resume()
         ...
         await ctx.send(embed = discord.Embed(title= "다시재생", description = musicnow[0]  + "을(를) 다시 재생했습니다.", color = 0xe9c949))
...
async def 노래끄기(ctx):
    if vc.is_playing():
        ...
        global number
        number = 0
        await ctx.send(embed = discord.Embed(title= "노래끄기", description = musicnow[0]  + "을(를) 종료했습니다.", color = 0xe94949))
    else:
        await ctx.send("지금 노래가 재생되지 않네요.")
